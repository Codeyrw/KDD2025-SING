    ...
    for epoch in range(40):
        ...
        for dataset_ind in range(len(datasets_tr)):
            ...
            for inner_steps in range(args.inner_steps):
                ...
                loss_generator = Mean + con_loss  - args.dist_weight * Var + 1.0*loss_str 
            ...
            loss_str_al += torch.mean(torch.ones(BY_pos.shape[0]).to(device) - BY_pos+ 0.5*BY_neg)#
            #------------------------------------------------------------------------------------------#
            for k in range(0, args.K):
                z_c_all=[]
                mask_per_view = generators_per_dataset(k,edge_index,dataset_tr.n)
            ...
            loss_classifier = Mean + Var + 0.005*loss_str_al/(args.K+1)
