    ...
    for epoch in range(args.epochs):
        ...
        for dataset_ind in range(len(datasets_tr)):
            ...
            for inner_steps in range(args.inner_steps):
                ...
                rep_load = z.detach().cpu().numpy()
                np.save('z_CBAS_concept.npy',rep_load)
            ...
            z_res_tmp = z_res[tr_id].repeat(idt.shape[0],1,1)
            idt_tmp = (idt.unsqueeze(2)).repeat(1,1,z.shape[1])#train_num*train_num*d
            z_c = torch.mean(z_res_tmp*idt_tmp,dim=1)
            ...
            for k in range(0, args.K):
                ...
                ##----------------structure align in graph------------------------------------------------#
                z_res_tmp = z_res[tr_id].repeat(idt.shape[0],1,1)
                z_c = torch.mean(z_res_tmp*idt_tmp,dim=1)#mean structural encodings for each sample 
                anchor = z_c[label_sign.to(device)]#C*d
                BY_pos = torch.matmul(z_res[tr_id]/(torch.norm(z_res[tr_id],dim=1).unsqueeze(1)+1e-10),anchor.T/(torch.norm(anchor,dim=1)+1e-10))[range(z_c.shape[0]),dataset_tr.label[tr_id].squeeze(1)]
