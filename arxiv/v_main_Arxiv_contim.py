...
def get_dataset(dataset_name, mol=None, ratio=None, sub_dataset=None, year=None,):
    ...
    if dataset_name == 'GOODArxiv':
        shift_type='concept'
        domain_type = 'time'
        print(shift_type,domain_type)
        dataset = load_dataset(name='GOODArxiv', dataset_root ='./data', domain = domain_type, shift =shift_type, generate=False)
...
if args.dataset == 'ogb-arxiv':
    ...
    datasets_tr = [get_dataset(dataset_name='GOODArxiv', mol='train')]
    datasets_val = [get_dataset(dataset_name='GOODArxiv', mol='val')]
    datasets_te = [get_dataset(dataset_name='GOODArxiv', mol='test')]
else:
    raise ValueError('Invalid dataname')
...
for run in range(0,args.runs):
    ...
    for epoch in range(205):
        ...
        for dataset_ind in range(len(datasets_tr)):
            ...
            tr_id = tr_id[0:int(0.5*tr_id.shape[0])]
            train_Y = Y[tr_id]
            ...
            while i < dataset_tr.label[tr_id].shape[0]:
            ...
            for inner_steps in range(args.inner_steps):
                ...
                loss_var_array.append(torch.sqrt(loss_og).view(-1))
                z_all.append(z)
                A =torch.sparse_coo_tensor(edge_index.cuda(), torch.ones(edge_index.shape[1]).cuda(),[dataset_tr.n,dataset_tr.n]).to(device)
                D = torch.sparse.sum(A,dim=[1]).to_dense().unsqueeze(1)
                z_res= z-torch.sparse.mm(A,z)/D
                ...
                for k in range(0, args.K):
                    ...
                    Dk = torch.sparse.sum(Ak,dim=[1]).to_dense()
                    rep_all.append(z-torch.sparse.mm(A,z)/(Dk.unsqueeze(1)))
                ...
                loss_generator = Mean  - args.dist_weight * Var + 1.5*loss_str #+ 1*con_loss
            ...
            z_res= z-torch.sparse.mm(A,z)/(D.unsqueeze(1))
            ...
            for k in range(0, args.K):
                ...
                loss_array.append(loss_local.view(-1))
                Ak =torch.sparse_coo_tensor(edge_index.cuda(), mask_per_view.cuda(),[dataset_tr.n,dataset_tr.n]).to(device)
                Dk = torch.sparse.sum(Ak.detach(),dim=[1]).to_dense().unsqueeze(1)
                z_res= z-torch.sparse.mm(Ak.detach(),z)/Dk
            ...
            loss_classifier = Mean + Var + 0.01*loss_str_al/(args.K+1)
