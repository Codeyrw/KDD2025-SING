...
def get_dataset(dataset_name, mol=None, ratio=None, sub_dataset=None, year=None,):
    ...
    if dataset_name == 'GOODCora':
        shift_type='covariate'
        domain_type = 'degree'
        print(shift_type,domain_type)
        dataset = load_dataset(name='GOODCora', dataset_root ='./data', domain = domain_type, shift =shift_type, generate=False)
...
if args.dataset == 'ogb-arxiv':
    ...
    datasets_tr = [get_dataset(dataset_name='GOODCora', mol='train')]
    datasets_val = [get_dataset(dataset_name='GOODCora', mol='val')]
    datasets_te = [get_dataset(dataset_name='GOODCora', mol='test')]
else:
    raise ValueError('Invalid dataname')
...
tr_id = tr_id[0:int(0.95*tr_id.shape[0])]
train_Y = Y[tr_id]
    ...
    for epoch in range(40):
        ...
        for dataset_ind in range(len(datasets_tr)):
            ...
            for inner_steps in range(args.inner_steps):
                ...
                z_res = z-torch.mm(A.to_dense(),z)/D
                ...
                for k in range(0, args.K):
                    ...
                    rep_all.append(z-torch.mm(Ak.to_dense(),z)/Dk)
                ...
                loss_generator = Mean + con_loss  - args.dist_weight * Var + 0.1*loss_str  
            ...
            loss_str_al += torch.mean(torch.ones(BY_pos.shape[0]).to(device) - BY_pos)#+ 0.1*BY_neg
            ...
            for k in range(0, args.K):
                ...
                mask[range(z_c.shape[0]),dataset_tr.label[tr_id].squeeze(1)]=0
                BY_neg = torch.mean(BY_gui*mask,dim=1)
                loss_str_al += torch.mean(torch.ones(BY_pos.shape[0]).to(device) - BY_pos)

            Loss = torch.cat(loss_array, dim=0)
            Var, Mean = torch.var_mean(Loss)
            optimizer_model.zero_grad()
            loss_classifier = Mean + Var + 0.06*loss_str_al/(args.K+1)
