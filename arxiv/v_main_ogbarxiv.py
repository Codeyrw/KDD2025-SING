...
from model_arxiv import Ours, Graph_Editer
...
tr_id = torch.randint(dataset_tr.n,size=(1,int(0.5*dataset_tr.n))).squeeze(0)
idt = torch.matmul(Y[tr_id],Y[tr_id].T) 
i=0
label_sign = []
init_label=0
...
for run in range(1,args.runs):
    ...
    for epoch in range(60):
        ...
        for dataset_ind in range(len(datasets_tr)):
            ...
            loss_str_al += torch.mean(torch.ones(BY_pos.shape[0]).to(device) - BY_pos+0.05*BY_neg)#
            #------------------------------------------------------------------------------------------#
